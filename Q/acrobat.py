# Q-learning adapted for acrobat
...
def set_up_agent(buckets):
    class AcrobotAgent():
        ...
        def __init__(self, num_episodes=1000):
            ...
            self.upper_bounds = [1, 1, 1, 1, self.env.observation_space.high[4], self.env.observation_space.high[5]]
            self.lower_bounds = [-1, -1, -1, -1, self.env.observation_space.low[4], self.env.observation_space.low[5]]
...
def test_agent(min_lr, min_epsilon, discount, decay, bucket_0, bucket_1, bucket_2, bucket_3, bucket_4, bucket_5):
    buckets = tuple([int(bucket_0), int(bucket_1), int(bucket_2), int(bucket_3), int(bucket_4), int(bucket_5)])   
    scores = []
    #test 5 agents
    for i in range(1):
...
#Bayes opt, comented out so we can test on opt params found

# pbounds = {'min_lr': (0.001, 0.5), 'min_epsilon': (0.001, 0.5), 'discount': (0.1, 0.9999), 'decay': (1, 200), 'bucket_0': (1, 30), 'bucket_1': (1, 30), 'bucket_2': (1, 30), 'bucket_3': (1, 30), 'bucket_4': (1, 30), 'bucket_5': (1, 30)}
...
# Test using opt buckets from acrobat opt
# Will test_agent on each set of params 100 times
params_opt_for_cartpole = []
params_opt_for_mountaincar = []
params_opt_for_acrobat = []

trials = 100
for i in range(trials):

    cartpole = test_agent(0.38732364823894777, 0.06306044698237305, 0.9560266938018456, 69.67505733621513, 9.99557425586167, 1, 1, 1, 19.6, 30)
    mountain_car = test_agent(0.04020428828777738, 0.41041559197591426, 0.5601072581830205, 111.67543600719513, 9.99557425586167, 1, 1, 1, 19.6, 30)
    acrobat = test_agent(0.001, 0.5, 0.9999, 42.14476816745908, 9.99557425586167, 1, 1, 1, 19.6, 30)

    print('Using opt for cartpole', cartpole)
    print('Using opt for mountain car', mountain_car)
    print('Using opt for acrobat', acrobat)

    params_opt_for_cartpole.append(cartpole)
    params_opt_for_mountaincar.append(mountain_car)
    params_opt_for_acrobat.append(acrobat)

print('rewards for agent trained on cartpole params = ', params_opt_for_cartpole)
print('rewards for agent trained on mountaincar params = ', params_opt_for_mountaincar)
print('rewards for agent trained on acrobat params = ', params_opt_for_acrobat)
